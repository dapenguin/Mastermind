 ...
 * @param  {Object} pegValueTotals    An object containing the total of each peg
 ...
 */
function getPartialMatches (exactMatchResults, codeGuess, numberOfPegs, secretCode, pegValueTotals) {
	...
	for (let i=0; i<numberOfPegs; i++){
		...
		if (guessMatches[i] === _MATCHES_NONE_){
			...
			for (let c=0; c<numberOfPegs; c++){
				if (!alreadyMatched[c] && codeGuess[i] === secretCode[c] && guessedPegValueTotals[secretCode[c]] < pegValueTotals[secretCode[c]]){
					guessMatches[i] = _MATCHES_PARTIAL_;
					alreadyMatched[c] = true;
					totalPartialMatches++;
					guessedPegValueTotals[secretCode[c]] = incrementPegValueTotal(guessedPegValueTotals[secretCode[c]]);
					break;
				}
			}
		}
