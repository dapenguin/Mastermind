// ToDo: See if we still need colourTotals
	...
	for (let i=0; i<numberOfPegs; i++){
		...
		if (guessMatches[i] === _MATCHES_NONE_){
			...
			for (let c=0; c<numberOfPegs; c++){
				...
				// console.log(`alreadyMatched[c]: ${alreadyMatched[c]}, codeGuess[i] === secretCode[c]: ${codeGuess[i] === secretCode[c]}, guessedColourTotals[secretCode[c]] < colourTotals[secretCode[c]]: ${guessedColourTotals[secretCode[c]] < colourTotals[secretCode[c]]}`);
				if (!alreadyMatched[c] && codeGuess[i] === secretCode[c] && guessedColourTotals[secretCode[c]] < colourTotals[secretCode[c]]){
					guessMatches[i] = _MATCHES_PARTIAL_;
					alreadyMatched[c] = true;
					totalPartialMatches++;
					guessedColourTotals[secretCode[c]] = incrementPegValueTotal(guessedColourTotals[secretCode[c]]);
					break;
				}
			}
		}
